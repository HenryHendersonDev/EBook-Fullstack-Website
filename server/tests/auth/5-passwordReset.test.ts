...
describe('POST /auth/password-reset', () => {
  ...
  it('Should Return 400 with Error Code INVALID_OTP | Using Cookie ', async () => {
    const imgDIR = path.join(__dirname, './avatar.jpg');
    const data = generateRandomData();
    const body = {
      email: data.email,
  ...
  it('Should Return 200 with Error Code SUCCESSFULLY_RESET_PASSWORD | Using Cookie ', async () => {
    await new Promise((resolve) => setTimeout(resolve, 2500));
    await deleteAllEmails();
    await new Promise((resolve) => setTimeout(resolve, 2500));
    const imgDIR = path.join(__dirname, './avatar.jpg');
